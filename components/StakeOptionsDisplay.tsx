...
export const StakeOptionsDisplay = ({
  ...
  const [ltcTokenAccount, setLtcTokenAccount] = useState<Account>()
  ...
  useEffect(() => {
    ...
    if (walletAdapter.publicKey) {
      const userStakeATA = getAssociatedTokenAddress(
        ...
        .then((account) => setLtcTokenAccount(account))
        .catch((error) => {
          console.log(error)
        })
    }
    ...
    >
      ...
      <VStack spacing={-1}>
        <Text color="white" as="b" fontSize="4xl">
          {`${Number(ltcTokenAccount?.amount ?? 0) / Math.pow(10, 2)} $LTC`}
        </Text>
        <Text color="bodyText">
          {stakeAccount?.stakeState.staked
            ? `${stakeAccount?.claimable().toPrecision(2)} $LTC earned`
            : "earn $LTC by staking"}
        </Text>
      </VStack>
      <Button
      ...
      >
        <Text as="b">
          {stakeAccount?.stakeState.staked ? "claim $LTC" : "stake otteeer"}
        </Text>
      </Button>
      {stakeAccount?.stakeState.staked ? (
